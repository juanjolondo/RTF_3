...
public class ArchivoPedidoClientes {
	...
	private Map<String, PedidoCliente> cache;

	public ArchivoPedidoClientes(){
		
		this.cache = new HashMap<String, PedidoCliente>();
	...
	public boolean guardar(PedidoCliente pc){
	...
	public PedidoCliente buscar(String consecutivo){
		
		llenarCache();		
		PedidoCliente pc = cache.get(consecutivo);		
		return pc;
	...
	public List<PedidoCliente> obtenerTodos(){
		...
		llenarCache();		
		List<PedidoCliente> pc = (List<PedidoCliente>) this.cache.values();		
		return pc;
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        PedidoCliente pc = parsePedidoCliente(Charset.forName(encoding).decode(buf));
		        buf.flip();
	...
	private PedidoCliente parsePedidoCliente(CharBuffer registro){
		...
		PedidoCliente ps = new PedidoCliente(id, descripcion, medioPago, precio, pagado, consecutivo, cantidades, estado, fecha, productos);
		return ps;
		
	}

	private String parseString(PedidoCliente ps) {
