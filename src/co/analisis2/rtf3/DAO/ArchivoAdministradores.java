...
import co.analisis2.rtf3.entidades.Administrador;
	...
	private Map<String, Administrador> cache;

	public ArchivoAdministradores(){
		
		this.cache = new HashMap<String, Administrador>();
	...
	public Administrador buscar(String id){
		...
		llenarCache();		
		Administrador cliente = cache.get(id);		
		return cliente;
	...
	public List<Administrador> obtenerTodos(){
		...
		llenarCache();		
		List<Administrador> administradores = (List<Administrador>) this.cache.values();		
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        Administrador admin = parseAdministrador(Charset.forName(encoding).decode(buf));
		        buf.flip();
	...
	private Administrador parseAdministrador(CharBuffer registro){
		...
		Administrador ad = new Administrador(nombres, apellidos, id, genero, tipoId, telefono, direccion, correo);
		return ad;
		
	}

	private String parseString(Administrador admin) {
