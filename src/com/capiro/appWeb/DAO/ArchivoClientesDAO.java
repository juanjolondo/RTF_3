...
public class ArchivoClientesDAO {
	...
	private Map<String, ClienteDTO> cache;

	public ArchivoClientesDAO(){
		
		this.cache = new HashMap<String, ClienteDTO>();
	...
	public boolean guardar(ClienteDTO fp){
	...
	public ClienteDTO buscar(String id){
		...
		llenarCache();		
		ClienteDTO cliente = cache.get(id);		
		return cliente;
	...
	public List<ClienteDTO> obtenerTodos(){
		...
		llenarCache();		
		List<ClienteDTO> clientes = (List<ClienteDTO>) this.cache.values();		
		return clientes;
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        ClienteDTO cliente = parseCliente(Charset.forName(encoding).decode(buf));
		        buf.flip();
	...
	private ClienteDTO parseCliente(CharBuffer registro){
		...
		ClienteDTO c = new ClienteDTO(nombres, apellidos, id, genero, tipoId, telefono, direccion, correo);
		return c;
		
	}

	private String parseString(ClienteDTO fp) {
