...
public class ArchivoPedidoClientesDAO {
	
	private static final String NOMBRE_ARCHIVO = "pedidosClientes";
	...
	private Map<String, PedidoClienteDTO> cache;

	public ArchivoPedidoClientesDAO(){
		
		this.cache = new HashMap<String, PedidoClienteDTO>();
	...
	public boolean guardar(PedidoClienteDTO pc){
		
		String registro = parseString(pc);
		
		byte data[] = registro.getBytes();
		ByteBuffer out = ByteBuffer.wrap(data);	
		try (FileChannel fc = (FileChannel.open(file, APPEND))) {
			fc.write(out);
			this.cache.put(pc.getConsecutivo(), pc);
			if(!guardarDescripcion(pc.getConsecutivo(), pc.getDescripcion()) || 
			   !guardarListas(pc.getConsecutivo(), pc.getListaProductos(), pc.getCantidad())){
				return false;
			}
			return true;
		} catch (IOException x) {
	...
	public PedidoClienteDTO buscar(String consecutivo){
		
		llenarCache();		
		PedidoClienteDTO pc = cache.get(consecutivo);		
		return pc;
		
	}
	
	public List<PedidoClienteDTO> obtenerTodos(){
		
		llenarCache();		
		List<PedidoClienteDTO> pc = (List<PedidoClienteDTO>) this.cache.values();		
		return pc;
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        PedidoClienteDTO pc = parsePedidoCliente(Charset.forName(encoding).decode(buf));
		        buf.flip();
		        cache.put(pc.getConsecutivo(), pc);		        
		    }
		} catch (IOException x) {
	...
	private PedidoClienteDTO parsePedidoCliente(CharBuffer registro){
		...
		PedidoClienteDTO ps = new PedidoClienteDTO(id, descripcion, medioPago, precio, pagado, consecutivo, cantidades, estado, fecha, productos);
		return ps;
		
	}

	private String parseString(PedidoClienteDTO ps) {
