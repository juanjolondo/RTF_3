...
import com.capiro.appWeb.entidades.ProveedorDTO;

public class ArchivoProveedoresDAO {
	...
	private Map<String, ProveedorDTO> cache;

	public ArchivoProveedoresDAO(){
		
		this.cache = new HashMap<String, ProveedorDTO>();
	...
	public boolean guardar(ProveedorDTO proveedor){
	...
	public ProveedorDTO buscar(String id){
		...
		llenarCache();		
		ProveedorDTO proveedor = cache.get(id);		
		return proveedor;
	...
	public List<ProveedorDTO> obtenerTodos(){
		...
		llenarCache();		
		List<ProveedorDTO> proveedores = (List<ProveedorDTO>) this.cache.values();		
		return proveedores;
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        ProveedorDTO prov = parseProveedor(Charset.forName(encoding).decode(buf));
		        buf.flip();
	...
	private ProveedorDTO parseProveedor(CharBuffer registro){
		...
		ProveedorDTO c = new ProveedorDTO(nombres, apellidos, id, genero, tipoId, telefono, direccion, correo);
		return c;
		
	}

	private String parseString(ProveedorDTO prov) {
