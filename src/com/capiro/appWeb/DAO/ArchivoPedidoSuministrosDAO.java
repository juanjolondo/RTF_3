...
import com.capiro.appWeb.entidades.PedidoSuministrosDTO;

public class ArchivoPedidoSuministrosDAO {
	...
	private Map<String, PedidoSuministrosDTO> cache;

	public ArchivoPedidoSuministrosDAO(){
		
		this.cache = new HashMap<String, PedidoSuministrosDTO>();
	...
	public boolean guardar(PedidoSuministrosDTO ps){
	...
	public PedidoSuministrosDTO buscar(String consecutivo){
		
		llenarCache();		
		PedidoSuministrosDTO ps = cache.get(consecutivo);		
		return ps;
		
	}
	
	public List<PedidoSuministrosDTO> obtenerTodos(){
		
		llenarCache();		
		List<PedidoSuministrosDTO> ps = (List<PedidoSuministrosDTO>) this.cache.values();		
		return ps;
	...
	private void llenarCache(){
		...
		try (SeekableByteChannel sbc = Files.newByteChannel(file)) {
		    ...
		    while (sbc.read(buf) > 0) {
		        buf.rewind();
		        PedidoSuministrosDTO ps = parsePedidoSuministros(Charset.forName(encoding).decode(buf));
		        buf.flip();
	...
	private PedidoSuministrosDTO parsePedidoSuministros(CharBuffer registro){
		...
		PedidoSuministrosDTO ps = new PedidoSuministrosDTO(id, descripcion, medioPago, precio, pagado, consecutivo, cantidades, estado, fecha, productos);
		return ps;
		
	}

	private String parseString(PedidoSuministrosDTO ps) {
